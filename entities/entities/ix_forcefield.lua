	...
	function ENT:Use(activator)
		...
		if (activator:IsCombine()) then
			...
			if (self:GetMode() > #MODES) then
				...
				self:EmitSound("ambient/levels/outland/combineshield_deactivate.wav", 90)
				self.dummy:EmitSound("ambient/levels/outland/combineshield_deactivate.wav", 90)

				Schema:AddCombineDisplayMessage("Attention Forcefield barrier mode changed to "..MODES[self:GetMode()][2], Color(255, 0, 0), true)
			else
				self:SetSkin(0)
				self.dummy:SetSkin(0)

				self:EmitSound("ambient/levels/outland/combineshieldactivate.wav", 90)
				self.dummy:EmitSound("ambient/levels/outland/combineshieldactivate.wav", 90)

				Schema:AddCombineDisplayMessage("Attention Forcefield barrier mode changed to "..MODES[self:GetMode()][2], Color(0, 180, 230), true)
			end

			self:EmitSound("buttons/combine_button5.wav", 90, 100 + (self:GetMode() - 1) * 15)
			self.dummy:EmitSound("buttons/combine_button5.wav", 90, 100 + (self:GetMode() - 1) * 15)

			activator:ChatNotify("Changed barrier mode to: "..MODES[self:GetMode()][2])

			Schema:SaveForceFields()
		else
			self:EmitSound("buttons/combine_button3.wav", 80)
		end
	end
