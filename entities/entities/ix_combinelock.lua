...
if (SERVER) then
	...
	function ENT:Toggle(ply)
		...
		if not (ply:IsCombine() and ply:IsCA() and (ply:Nick():find("CP:") or ply:Nick():find("OTA:")) and (not ply:Nick():find("i5") and not ply:Nick():find("i4") and not ply:Nick():find("RCT"))) or ply:IsCA() then
			self:DisplayError()
		...
		if self:GetLocked() then
			ply:ChatNotify("You unlocked the combine lock that is attached to the door.")
		else
			ply:ChatNotify("You locked the combine lock that is attached to the door.")
		end
